<template lang="pug">
	div
		div.columns(class="main mt-6")
			...
			div.tile
				div(class='tile is-parent' v-if="info")
					article(class="tile is-child notification is-success")
						p.title About 
						p.subtitle Schedule
						div.content 
							|Opening time at {{info.open}}
							|Closing time at {{info.close}}
				div(class='tile is-parent' v-if="staff")
					article(class="tile is-child notification is-success")
						p.title About 
						p.subtitle {{about}}
						div.content {{info.open}}
				div(class='tile is-parent'v-if="history")
					article(class="tile is-child notification is-success")
						p.title About 
						p.subtitle {{about}}
						div.content {{info.open}}
				div(class='tile is-parent' v-if="direction")
					article(class="tile is-child notification is-success")
						p.title About 
						p.subtitle {{about}}
						div.content {{info.open}}
