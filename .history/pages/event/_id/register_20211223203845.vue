<template lang="pug">
	div.container
		...
		div
			section
				form
					b-field(label="First Name" :label-position="form.firstName!=''?'on-border':'inside'" :type="submit==false?(form.firstName !=''? 'is-success':''):(form.firstName !=''?'is-success':'is-danger')"  :message="submit&&form.firstName==''?'Most complete this field':''")
					...
					b-field(label="Email" :label-position="form.email!=''?'on-border':'inside'" :type="submit==false?(form.email !=''? 'is-success':''):(form.email !=''?'is-success':'is-danger')" )
